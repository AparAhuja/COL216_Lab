...
bool addToMemory(string line, REGI& rf, int line_number, int procNum) {
    ...
    if (label.second.second == false)
        ...
        if (label.second.first == true) {
            ...
            if (rf.labels.find(label.first.first) != rf.labels.end()) {
                ...
                cout << "Processor " + to_string(procNum) + " raised Error: Same label used to represent different addresses.\nLabel repeated: " << label.first.first << ": at line number: " << line_number << "\n";
                return false;
            }
            else {
                // add label to map
                rf.labels.insert({ label.first.first, rf.PC });
            }
        }
    }
    ...
    if (!flag) {
        ...
        cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 1: At line number: " << line_number << ": " << line << "\n";
        return false;
    }
    else {
        ...
        if (args.size() == 0)
            ...
            if (rf.ins_map.find(args[0]) == rf.ins_map.end()) {
                // invalid instruction
                cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 2: At line number: " << line_number << ": " << line << "\n";
                return false;
            }
            // else store the instruction in memory
            int ins = rf.ins_map[args[0]];
            rf.insMemory[rf.PC] = ins;
            ...
            if (ins == 6) {
                if (len != 2) {
                    ...
                    cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 3: At line number: " << line_number << ": " << line << "\n";
                    return false;
                }
                else {
                    if (!isInteger(args[1])) {
                        if (!validId(args[1])) {
                            ...
                            cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 4: At line number: " << line_number << ": " << line << "\n";
                            return false;
                        }
                        // else, it is a valid id, push it
                        rf.label.push_back(make_pair(args[1], make_pair(rf.PC + 1, line_number)));
                    }
                    else {
                        long long addr = stoi(args[1]);
                        if (addr < 0 || addr >= 67108864) {
                            // address starts from 0 and max integer that can be stored is 2^26 - 1
                            cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 5: At line number: " << line_number << ": " << "\n";
                            return false;
                        }
                        // else store the value (absolute address) in memory
                        rf.insMemory[rf.PC + 1] = 4 * addr;
                    }
                }
            }
            ...
            //add sub mul slt
                ...
                if (len != 4) {
                    ...
                    cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 6: At line number: " << line_number << ": " << line << "\n";
                    return false;
                }
                else {
                    ...
                    if (rf.reg_map.find(args[1]) == rf.reg_map.end() || rf.reg_map.find(args[2]) == rf.reg_map.end() || rf.reg_map.find(args[3]) == rf.reg_map.end()) {
                        ...
                        cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 7: At line number: " << line_number << ": " << line << "\n";
                        return false;
                    }
                    else {
                        // store register numbers in memory
                        rf.insMemory[rf.PC + 1] = rf.reg_map[args[1]];
                        rf.insMemory[rf.PC + 2] = rf.reg_map[args[2]];
                        rf.insMemory[rf.PC + 3] = rf.reg_map[args[3]];
                        int temp = rf.insMemory[rf.PC + 1];
                        if (temp == 1 || temp == 26 || temp == 27 || temp == 28) {
                            cout << "Processor " + to_string(procNum) + " raised Access Error: Trying to access reserved register: At line number: " << line_number << ": " << line << "\n";
                            return false;
                        }
                        temp = rf.insMemory[rf.PC + 2];
                        if (temp == 1 || temp == 26 || temp == 27 || temp == 28) {
                            cout << "Processor " + to_string(procNum) + " raised Access Error: Trying to access reserved register: At line number: " << line_number << ": " << line << "\n";
                            return false;
                        }
                        temp = rf.insMemory[rf.PC + 3];
                        if (temp == 1 || temp == 26 || temp == 27 || temp == 28) {
                            cout << "Processor " + to_string(procNum) + " raised Access Error: Trying to access reserved register: At line number: " << line_number << ": " << line << "\n";
                            return false;
                        }
            ...
            //beq bne addi
                ...
                if (len != 4) {
                    ...
                    cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 8: At line number: " << line_number << ": " << line << "\n";
                    return false;
                }
                else {
                    if (ins == 9) {
                        ...
                        if (rf.reg_map.find(args[1]) == rf.reg_map.end() || rf.reg_map.find(args[2]) == rf.reg_map.end() || !isInteger(args[3])) {
                            ...
                            cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 9: At line number: " << line_number << ": " << line << "\n";
                            return false;
                        }
                        else {
                            ...
                            if (immediate < -32768 || immediate > 32767) {
                                cout << "Processor " + to_string(procNum) + " raised Immediate Overflow ERROR: At line number: " << line_number << ": " << "\n";
                                return false;
                            }
                            // else
                            ...
                            rf.insMemory[rf.PC + 1] = rf.reg_map[args[1]];
                            rf.insMemory[rf.PC + 2] = rf.reg_map[args[2]];
                            rf.insMemory[rf.PC + 3] = immediate;
                            int temp = rf.insMemory[rf.PC + 1];
                            if (temp == 1 || temp == 26 || temp == 27 || temp == 28) {
                                cout << "Processor " + to_string(procNum) + " raised Access Error: Trying to access reserved register: At line number: " << line_number << ": " << line << "\n";
                                return false;
                        ...
                        if (rf.reg_map.find(args[1]) == rf.reg_map.end() || rf.reg_map.find(args[2]) == rf.reg_map.end()) {
                            // invalid arguments, syntax error
                            cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 10: At line number: " << line_number << ": " << line << "\n";
                            return false;
                        }
                        else {
                            // store register
                            ...
                            if (isInteger(args[3])) {
                                ...
                                rf.insMemory[rf.PC + 3] = rf.PC + 4 + 4 * immediate;
                            }
                            else {
                                if (validId(args[3])) {
                                    rf.label.push_back(make_pair(args[3], make_pair(rf.PC + 3, line_number)));
                                }
                                else {
                                    // invalid label, raise error
                                    cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 11: At line number: " << line_number << ": " << line << "\n";
                                    return false;
                                }
                            }
                        }
            ...
            //sw lw
                ...
                if (len != 4) {
                    ...
                    cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 12: At line number: " << line_number << ": " << line << "\n";
                    return false;
                }
                else {
                    ...
                    if (rf.reg_map.find(args[1]) == rf.reg_map.end() || !isInteger(args[2]) || rf.reg_map.find(args[3]) == rf.reg_map.end()) {
                        // invalid arguments, syntax error
                        cout << "Processor " + to_string(procNum) + " raised SYNTAX ERROR 13: At line number: " << line_number << ": " << line << "\n";
                        return false;
                    }
                    else {
                        ...
                        // store register numbers and immediate in memory
                        rf.insMemory[rf.PC + 1] = rf.reg_map[args[1]];
                        rf.insMemory[rf.PC + 2] = immediate;
                        rf.insMemory[rf.PC + 3] = rf.reg_map[args[3]];
                        int temp = rf.insMemory[rf.PC + 1];
                        if (temp == 1 || temp == 26 || temp == 27 || temp == 28) {
                            cout << "Processor " + to_string(procNum) + " raised Access Error: Trying to access reserved register: At line number: " << line_number << ": " << line << "\n";
...
bool linker(REGI& rf, int procNum) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (rf.labels.find(label.first) == rf.labels.end()) {
            ...
            cout << "Processor " + to_string(procNum) + " raised Linking Error: Unable to resolve label used on line: " << label.second.second << "\n";
            return false;
        }
        else {
            // resolve label
            int label_loc = rf.labels[label.first];
            rf.insMemory[label.second.first] = label_loc;
        }
    }
    // all labels resolved
    return true;
